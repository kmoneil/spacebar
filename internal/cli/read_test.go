...
func TestARowIsWrittenAsItArrivesRatherThanAtTheEnd(t *testing.T) {
	...
	if err := stream(r, source, rowForSpace); err != nil {
...
func TestTheRowShapesAreTheContractAnAgentParses(t *testing.T) {
	space, _ := rowForSpace(chat.Space{
		Name:            "spaces/AAA",
		DisplayName:     "Ops",
		SpaceType:       "SPACE",
	...
	member, _ := rowForMember(chat.Membership{
	...
	message, _ := rowForMessage(chat.Message{
...
func TestAMissingSenderDoesNotProduceABlankColumnWithNoExplanation(t *testing.T) {
	_, cells := rowForMessage(chat.Message{
		Name:   "spaces/AAA/messages/BBB",
	...
	_, cells = rowForMessage(chat.Message{Name: "spaces/AAA/messages/BBB"})
	if cells[1] != "" {
...
func TestTwoDirectMessagesAreNotTheSameRow(t *testing.T) {
	withApp, appCells := rowForSpace(chat.Space{
		Name:            "spaces/BBB",
		SpaceType:       "DIRECT_MESSAGE",
		SingleUserBotDm: true,
		LastActiveTime:  "2026-04-17T11:29:52.558415Z",
	})
	withPerson, personCells := rowForSpace(chat.Space{
		Name:           "spaces/CCC",
		SpaceType:      "DIRECT_MESSAGE",
